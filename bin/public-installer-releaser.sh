...
if [[ ${UPLOAD_NEW_PUBLIC_INSTALLER} == 'true' ]]; then
  ...
  aws s3 cp --acl public-read-write --content-type text/plain ../src/build/public-installer.sh "s3://armory-web/install/release/kubernetes-installer/public-installer.sh"
fi
